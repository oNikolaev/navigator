...
Асинх Процедура СохранитьПроектНаКлиенте()
	...
	Если Проекты.ПолучитьЭлементы().Количество() = 0 Тогда
		ПоказатьПредупреждение(, "Нет проектов для сохранения.");
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрока = Элементы.Проекты.ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		ПоказатьПредупреждение(, "Не выбран проект для сохранения.");
		Возврат;
	КонецЕсли;
	
	стр_Проекты = Проекты.НайтиПоИдентификатору(ТекущаяСтрока);
	Если Не (стр_Проекты.Тип = "Проект") Тогда
		ПоказатьПредупреждение(, "Не выбран проект для сохранения.");
		Возврат;
	КонецЕсли;
	...
	// Подготовить данные проекта в виде json
	ДанныеПроекта = ДанныеТекущегоПроектаКакJSON();
	Если Не ЗначениеЗаполнено(ДанныеПроекта) Тогда
		ПоказатьПредупреждение(, "Нет данных проекта для сохранения. Сохранение данных проекта не выполнено!");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция ДанныеТекущегоПроектаКакJSON()
	...
	ДобавитьЭлементыСтруктурыПроекта(стр_Проекты, СтруктураПроекта);
	
КонецФункции

&НаКлиенте
Процедура ДобавитьЭлементыСтруктурыПроекта(ЭлементПроекта, СтруктураПроекта)
	
	Для каждого ДочернийЭлемент Из ЭлементПроекта.ПолучитьЭлементы() Цикл
		
		Если ДочернийЭлемент.ПолучитьЭлементы().Количество() > 0 Тогда
			ДобавитьЭлементыСтруктурыПроекта(ДочернийЭлемент, СтруктураПроекта);
		Иначе
			Если ДочернийЭлемент.Тип = "Группа" Тогда
			Иначе
				СтруктураЭлементаПроекта =
					Новый Структура("Тип,НомерКартинки,Представление,Имя,Значение,Свойства", 
						ДочернийЭлемент.Тип, ДочернийЭлемент.НомерКартинки, ДочернийЭлемент.Представление, ДочернийЭлемент.Имя, ДочернийЭлемент.Значение, ДочернийЭлемент.Свойства);
					
				СтруктураПроекта.Вставить(ДочернийЭлемент.Имя, СтруктураЭлементаПроекта);
			КонецЕсли;
