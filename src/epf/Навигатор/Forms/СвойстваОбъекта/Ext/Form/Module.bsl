...
Процедура ОтобразитьСтатистикуНаСервере()
	...
	Если Метаданные[ИмяКоллекции][ИмяОбъектаБазы].Иерархический Тогда
		СКД = Навигатор.ПолучитьМакет("СтатистикаПоИерархическомуСправочнику");
	Иначе
		СКД = Навигатор.ПолучитьМакет("СтатистикаПоСправочнику");
	КонецЕсли;
	
	СКД.НаборыДанных.ДанныеСтатистики.Запрос = СтрЗаменить(СКД.НаборыДанных.ДанныеСтатистики.Запрос, "Номенклатура", ИмяОбъектаБазы);
