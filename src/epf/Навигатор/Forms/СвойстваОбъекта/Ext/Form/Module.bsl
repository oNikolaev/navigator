...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьВсеПодсистемы();
	ЗаполнитьВсеРегистраторы();
	
	Навигатор = РеквизитФормыВЗначение("Объект");
	
	Если ИмяЭлементаКоллекции = "Справочник" Тогда
		
		Элементы.СтраницаВладельцы.Видимость = Истина;
		
		Если Метаданные[ИмяКоллекции][ИмяОбъектаБазы].Владельцы.Количество() > 0 Тогда
			Для каждого эл_Владелец Из Метаданные[ИмяКоллекции][ИмяОбъектаБазы].Владельцы Цикл
				//
			КонецЦикла;
		КонецЕсли;
		
		Элементы.СтраницаСтатистика.Видимость = Истина;
...
Процедура СтраницыФормыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаСтатистика Тогда
		ОтобразитьСтатистикуНаСервере();
	КонецЕсли;
...
#Область РегистраторыРегистра

&НаКлиенте
Процедура РегистраторыРегистраВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Оповестить("ОткрытьСписокЭлементов", Элементы.РегистраторыРегистра.ТекущиеДанные.ОпределительСписка, "Навигатор_ФормаСвойстваОбъекта");
	
КонецПроцедуры

#КонецОбласти

#Область СтатистикаПоОбъекту

#Область Команды

&НаКлиенте
Процедура КомандаОтобразитьСтатистику(Команда)
...
&НаСервере
Процедура ОтобразитьСтатистикуНаСервере()
	
	Навигатор = РеквизитФормыВЗначение("Объект");
	
	СКД = Навигатор.ПолучитьМакет("СтатистикаПоОбъектуМетаданных");
	
	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных();
	
	Если ИмяЭлементаКоллекции = "Справочник" Тогда
		
		Иерархический = Метаданные[ИмяКоллекции][ИмяОбъектаБазы].Иерархический;
		Подчиненный   = (Метаданные[ИмяКоллекции][ИмяОбъектаБазы].Владельцы.Количество() > 0);
		
		Если Иерархический И Подчиненный Тогда
			
			КомпоновщикНастроек.ЗагрузитьНастройки(СКД.ВариантыНастроек.Найти("СправочникИерархияПодчиненный").Настройки);
			СКД.НаборыДанных.СправочникИерархияПодчиненный.Запрос = 
				СтрЗаменить(СКД.НаборыДанных.СправочникИерархияПодчиненный.Запрос, "#ИмяСправочника", СтрШаблон("Справочник.%1", ИмяОбъектаБазы));
			
		ИначеЕсли Иерархический Тогда
			
			КомпоновщикНастроек.ЗагрузитьНастройки(СКД.ВариантыНастроек.Найти("СправочникИерархия").Настройки);
			СКД.НаборыДанных.СправочникИерархия.Запрос = 
				СтрЗаменить(СКД.НаборыДанных.СправочникИерархия.Запрос, "#ИмяСправочника", СтрШаблон("Справочник.%1", ИмяОбъектаБазы));
				
		ИначеЕсли Подчиненный Тогда
			
			КомпоновщикНастроек.ЗагрузитьНастройки(СКД.ВариантыНастроек.Найти("СправочникПодчиненный").Настройки);
			СКД.НаборыДанных.СправочникПодчиненный.Запрос = 
				СтрЗаменить(СКД.НаборыДанных.СправочникПодчиненный.Запрос, "#ИмяСправочника", СтрШаблон("Справочник.%1", ИмяОбъектаБазы));
				
		Иначе
			
			КомпоновщикНастроек.ЗагрузитьНастройки(СКД.ВариантыНастроек.Найти("Справочник").Настройки);
			СКД.НаборыДанных.Справочник.Запрос = 
				СтрЗаменить(СКД.НаборыДанных.Справочник.Запрос, "#ИмяСправочника", СтрШаблон("Справочник.%1", ИмяОбъектаБазы));
	...
	СтатистикаПоОбъекту.Очистить();
	
	ПроцессорВывода = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВТабличныйДокумент();
	ПроцессорВывода.УстановитьДокумент(СтатистикаПоОбъекту);
...
&НаСервере
Процедура ЗаполнитьВсеРегистраторы()
	
	РегистраторыРегистра.Очистить();
	
	ом_Объект = Метаданные[ИмяКоллекции][ИмяОбъектаБазы];
	
	Если ИмяКоллекции = "РегистрыНакопления" 
		Или ИмяКоллекции = "РегистрыСведений" Или ИмяКоллекции = "РегистрыБухгалтерии" 
		Или ИмяКоллекции = "РегистрыРасчета" Тогда
		
		Если ИмяКоллекции = "РегистрыСведений" Тогда
			
			Если ом_Объект.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
				Элементы.СтраницаРегистраторы.Видимость = Ложь;
				Возврат;
			КонецЕсли;
			
		КонецЕсли;
		
		Для каждого ом_Документ Из Метаданные.Документы Цикл
			
			Если Не ом_Документ.Движения.Содержит(ом_Объект) Тогда
				Продолжить;
			КонецЕсли;
			
			стр_Регистраторы = РегистраторыРегистра.Добавить();
			стр_Регистраторы.Имя = СтрШаблон("Документ.%1", ом_Документ.Имя);
			стр_Регистраторы.Синоним = ом_Документ.Синоним;
			стр_Регистраторы.ЯвляетсяРегистратором = Истина;
			стр_Регистраторы.ОпределительСписка = СтрШаблон("Документ.%1", ом_Документ.Имя);
			
		КонецЦикла;
		
		РегистраторыРегистра.Сортировать("Имя ВОЗР");
		
		Если РегистраторыРегистра.Количество() > 0 Тогда
			Элементы.СтраницаРегистраторы.Заголовок = СтрШаблон("Регистраторы (%1)", РегистраторыРегистра.Количество());
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьВсеПодсистемы()
	
	мд_ОбъектМетаданных = Метаданные[ИмяКоллекции][ИмяОбъектаБазы];
	дз_СвязанныеПодсистемы = РеквизитФормыВЗначение("СвязанныеПодсистемы");
	
	Для каждого Подсистема Из Метаданные.Подсистемы Цикл
		ДобавитьСведенияПоПодсистемеРекурсивно(Подсистема, дз_СвязанныеПодсистемы.Строки, мд_ОбъектМетаданных);
	КонецЦикла;
	
	ЗначениеВРеквизитФормы(дз_СвязанныеПодсистемы, "СвязанныеПодсистемы");
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьСведенияПоПодсистемеРекурсивно(Подсистема, дз_СвязанныеПодсистемы, мд_ОбъектМетаданных)
	
	Если Подсистема.Подсистемы.Количество() = 0 Тогда
		Если Не Подсистема.Состав.Содержит(мд_ОбъектМетаданных) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	стр_СвязанныеПодсистемы = дз_СвязанныеПодсистемы.Добавить();
	ЗаполнитьЗначенияСвойств(стр_СвязанныеПодсистемы, Подсистема);
	
	Для каждого вл_Подсистема Из Подсистема.Подсистемы Цикл
		ДобавитьСведенияПоПодсистемеРекурсивно(вл_Подсистема, стр_СвязанныеПодсистемы.Строки, мд_ОбъектМетаданных);
	КонецЦикла;
	
	Если стр_СвязанныеПодсистемы.Строки.Количество() = 0 Тогда
		Если Не Подсистема.Состав.Содержит(мд_ОбъектМетаданных) Тогда
			дз_СвязанныеПодсистемы.Удалить(стр_СвязанныеПодсистемы);
		КонецЕсли;
	КонецЕсли;
