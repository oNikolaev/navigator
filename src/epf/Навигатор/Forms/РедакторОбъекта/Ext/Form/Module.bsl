...
Процедура РазобратьОписаниеСвязанныхДанныхНаСервере()
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		СоставОписания = СтрРазделить(Строка, ".", Ложь);
...
Процедура ЗаполнитьВсеРеквизиты()
	...
	Системные.Представление     = СтрШаблон("СистемныеРеквизиты (%1)", 2);
	...
	Системные.Представление = СтрШаблон(
	...
	Если Метаданные.ОбщиеРеквизиты.Количество() > 0 Тогда
		...
		Если Не (Общие = Неопределено) Тогда
			Общие.Представление = СтрШаблон("ОбщиеРеквизиты (%1)", Общие.Строки.Количество());
		КонецЕсли;
	...
	Если МетаданныеОбъекта.СтандартныеРеквизиты.Количество() > 0 Тогда
		...
		Стандартные.Представление     = СтрШаблон("СтандартныеРеквизиты (%1)", МетаданныеОбъекта.СтандартныеРеквизиты.Количество());
	...
	Если МетаданныеОбъекта.Реквизиты.Количество() > 0 Тогда
		...
		Основные.Представление     = СтрШаблон("Реквизиты (%1)", МетаданныеОбъекта.Реквизиты.Количество());
		...
		Для каждого ТаблЧастьМд Из ОбъектМд.ТабличныеЧасти Цикл
			...
			ПредставлениеТаблЧасти = 
				?(КоличествоСтрокТаблЧасти > 0, 
					СтрШаблон("%1 (%2)", ТаблЧастьМд.Имя, КоличествоСтрокТаблЧасти), 
					ТаблЧастьМд.Имя
				);
...
Процедура ЗаполнитьСвязанныеОбъекты()
	...
	Элементы.СтраницаСвязанныеОбъекты.Видимость = Ложь; // Пока не реализовано
	...
	Если ЭтоДокумент Тогда
		...
		Для каждого РегистрМД Из ОбъектБазыМД.Движения Цикл
			...
			ПредставлениеРегистра = 
				?(КолЗаписей > 0, 
				СтрШаблон("%1 (%2)", РегистрМД.Имя, КолЗаписей), 
				РегистрМД.Имя
			);
			...
			Для каждого ОписаниеКоллекции Из ПереченьКоллекций Цикл
				...
				стр_Кл.Представление = СтрШаблон(
...
Процедура УстановитьЗаголовокСтраницыРеквизиты()
	...
	ЗаголовокСтраницы = СтрШаблон("Реквизиты (%1)", РеквизитФормыВЗначение("ВсеРеквизиты").Строки.Количество());
...
Процедура УстановитьЗаголовокСтраницыТабличныеЧасти()
	...
	ЗаголовокСтраницы = СтрШаблон("Табличные части (%1)", РеквизитФормыВЗначение("ВсеТабличныеЧасти").Строки.Количество());
...
Процедура УстановитьЗаголовокСтраницыДвижения()
	...
	ЗаголовокСтраницы = СтрШаблон("Регистры накопления (%1)", КоличествоРН);
	...
	ЗаголовокСтраницы = СтрШаблон("Регистры сведений (%1)", КоличествоРС);
	...
	ЗаголовокСтраницы = СтрШаблон("Регистры бухгалтерии (%1)", КоличествоРБ);
	...
	ЗаголовокСтраницы = СтрШаблон("Регистры расчета (%1)", КоличествоРР);
	...
	Если КоличествоРегистровВсего > 0 Тогда
		ЗаголовокСтраницыДвижения = СтрШаблон("Движения (%1)", КоличествоРегистровВсего);
	Иначе
...
Процедура УстановитьЗаголовокСтраницыИзмененияНаУзлахПлановОбмена()
	...
	Элементы.СтраницаИзмененияНаУзлахПлановОбмена.Заголовок = ?(КолЭлементов > 0, СтрШаблон("Планы обмена (%1)", КолЭлементов), "Планы обмена");
...
Процедура ОтобразитьДанныеТабличнойЧасти(ИмяТабличнойЧасти)
	...
	Если КолЗаписейТаблЧасти > 0 Тогда
		СтраницаДанныеТабличнойЧасти.Заголовок = СтрШаблон("%1 (%2)", ИмяТабличнойЧасти, КолЗаписейТаблЧасти);
	Иначе
		СтраницаДанныеТабличнойЧасти.Заголовок = СтрШаблон("%1", ИмяТабличнойЧасти);
	КонецЕсли;
	...
	Для Каждого РеквизитТЧ Из ОбъектМд.ТабличныеЧасти[ИмяТабличнойЧасти].СтандартныеРеквизиты Цикл
		...
		ИмяРеквизитаКолонка = ИмяРеквизитаФормыКолонкиТаблицыДанныхОбъекта("ТабличнаяЧасть", ИмяТабличнойЧасти, РеквизитТЧ.Имя);
		ПутьКДаннымДляКолонки = СтрШаблон("%1.%2", ИмяРеквизита, ИмяРеквизитаКолонка);
...
Процедура ОтобразитьДанныеДвиженийПоРегистру(ТипРегистра, ИмяРегистра)
	...
	Для Каждого СтандартныйРеквизит Из Метаданные[ИмяКоллекции][ИмяРегистра].СтандартныеРеквизиты Цикл
		...
		ИмяРеквизитаКолонка = ИмяРеквизитаФормыКолонкиТаблицыДанныхОбъекта(ТипРегистра, ИмяРегистра, СтандартныйРеквизит.Имя);
		ПутьКДаннымДляКолонки = СтрШаблон("%1.%2", ИмяДинамическогоСписка, ИмяРеквизитаКолонка);
...
Процедура ОтобразитьДанныеСвязанногоСправочника(ИмяПодчиненногоОбъекта)
	...
	ЧастиИмени = СтрРазделить(ИмяПодчиненногоОбъекта, ".", Ложь);
...
Процедура ДобавитьПолеРегистраБухгалтерииВСхемуЗапроса(ЗапросВыбораСхемыЗапроса,
	...
	Если (ТипОбработки = "ДтКт") Тогда
		...
		Для Каждого СуффиксПоля Из СуффиксыПоля Цикл
			...
			ИмяПоляДляВключения = СтрШаблон("%1%2", ИмяПоля, СуффиксПоля);
		...
		Для Каждого ИмяПоляСубконто Из ИменаПолейСубконто Цикл
			...
			Для НомерСубконто = 1 По МаксКоличествоСубконто Цикл
				...
				Для Каждого СуффиксПоля Из СуффиксыПоля Цикл
					...
					ИмяВыбираемогоПоля = СтрШаблон("%1%2%3", ИмяПоляСубконто, СуффиксПоля, НомерСубконто);
...
Процедура ДобавитьРеквизитСпискаРегистраБухгалтерии(ДобавляемыеРеквизиты,
	...
	Если (ТипОбработки = "ДтКт") Тогда
		...
		Для Каждого СуффиксПоля Из СуффиксыПоля Цикл
			...
			ИмяРеквизитаДтКт = СтрШаблон("%1%2", ИмяРеквизита, СуффиксПоля);
		...
		Для Каждого ИмяПоляСубконто Из ИменаПолейСубконто Цикл
			...
			Для НомерСубконто = 1 По МаксКоличествоСубконто Цикл
				...
				Для Каждого СуффиксПоля Из СуффиксыПоля Цикл
					...
					ИмяРеквизитаСубконто = СтрШаблон("%1%2%3", ИмяПоляСубконто, СуффиксПоля, НомерСубконто);
...
Процедура ПерейтиПоНавигационнойСсылкеНаЭлемент()
	...
	Если (Не ЗначениеЗаполнено(СсылкаНаЭлемент)) Тогда
		...
		ТекстСообщения = СтрШаблон("По ссылке '%1' не удалось найти элемент базы.", НавигационнаяСсылкаНаЭлемент);
	...
	Если ОбъектПоСсылке = Неопределено Тогда
		...
		ТекстСообщения = СтрШаблон("По ссылке '%1' не удалось получить данные объекта.", НавигационнаяСсылкаНаЭлемент);
		Сообщить(ТекстСообщения);
	...
	Для Каждого ОпределительПоля Из ОпределителиПолей Цикл
		...
		Для Каждого СуффиксПоля Из СуффиксыПолей Цикл
			...
			Для НомерСубконто = 1 По КолСубконто Цикл
				...
				ИмяПоля = СтрШаблон("РегистрБухгалтерии%1%2%3%4%5%6", РЧИ, ИмяРегистраБухгалтерии, РЧИ, ОпределительПоля, СуффиксПоля, НомерСубконто); // "РегистрБухгалтерии_Хозрасчетный_ВидСубконтоДт1"
	...
	Для Каждого КлючЗначение Из ПоляСубконто Цикл
		...
		Для Каждого СуффиксПоля Из СуффиксыПолей Цикл
			...
			Если (ОписаниеРазделаПолей[ИмяПоляХраненияГруппы]) = Неопределено Тогда
				...
				ИмяГруппы = СтрШаблон("%1%2Группа%3%4", ИмяЭлементаСписка, РЧИ, РазделПолей, СуффиксПоля); // "РегистрБухгалтерии_Хозрасчетный_ГруппаВидСубконтоДт"				
...
Процедура ВывестиПодчиненныеОбъекты(ТекущийОбъект, ДеревоРодитель)
	...
	Для Каждого КлючИЗначение Из КэшПоТипамОбъектов Цикл
		...
		МассивИмениОбъекта = СтрРазделить(КлючИЗначение.Ключ, ".", Ложь);
...
Функция ТекстЗаголовкаФормы()
	...
	Результат = СтрШаблон("%1 %2", "(...)", ОбъектБазы);
...
Функция ТекстЗапросаТаблицыДвиженийРегистраБухгалтерииОбъектаКонфигурации(ИмяРегистраБухгалтерии)
	...
	// Добавить источник в запрос.
	ПолноеИмяТаблицыРегистра = СтрШаблон("РегистрБухгалтерии.%1.ДвиженияССубконто", ИмяРегистраБухгалтерии);
...
Функция ИмяСтраницыТаблицыОбъекта(ТипТаблицы, ИмяТаблицы)
	...
	Результат = СтрШаблон("Страница" + РЧИ + "%1"
