...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ОбъектБазыВыгрузка) Тогда
	...
	Если ТипЗнч(Параметры.ОбъектыБазы) = Тип("Массив") Тогда
		
		Для каждого ОбъектБазы Из Параметры.ОбъектыБазы Цикл
			
			Если СписокОбъектовДляВыгрузки.НайтиСтроки(Новый Структура("ОбъектБазыВыгрузка", ОбъектБазы)).Количество() = 0 Тогда
				
				стр_СписокОбъектовДляВыгрузки = СписокОбъектовДляВыгрузки.Добавить();
				стр_СписокОбъектовДляВыгрузки.ОбъектБазыВыгрузка = ОбъектБазы;
				стр_СписокОбъектовДляВыгрузки.Пометка = Истина;
				стр_СписокОбъектовДляВыгрузки.ВыгружатьПоСсылкам = Истина;
				
			КонецЕсли;
			
		КонецЦикла;
		
		Элементы.СтраницыВыгрузка.ТекущаяСтраница = Элементы.СтраницаВыгрузкаСписка;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли Источник = "ОсновнаяФорма" Тогда
		
		Если ИмяСобытия = "ВыбраныЭлементыДляВыгрузки" Тогда
			
			Для каждого ОбъектБазы Из Параметр.ОбъектыБазы Цикл
...
#Область ОбъектыБазыЗагрузка

&НаКлиенте
Процедура ОбъектыБазыЗагрузкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ПараметрыФормы = Новый Структура("ОбъектБазы", Элемент.ТекущиеДанные.ОбъектБазыЗагрузка);
...
&НаКлиенте
Процедура КомандаОбъектыБазыЗагрузкаОткрытьВРедактореОбъекта(Команда)
	
	#Область Предусловия
	
	ТекущиеДанные = Элементы.ОбъектыБазыЗагрузка.ТекущиеДанные;
	...
	ПараметрыФормы = Новый Структура("ОбъектБазы", ТекущиеДанные.ОбъектБазыЗагрузка);
...
#Область СписокОбъектовДляВыгрузки

#Область Команды

&НаКлиенте
Процедура КомандаСписокОбъектовДляВыгрузкиПодбор(Команда)
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаСписокОбъектовДляВыгрузкиУстановитьПометки(Команда)
	
	Для каждого стр Из СписокОбъектовДляВыгрузки Цикл
		стр.Пометка = Истина;
	КонецЦикла; 
...
&НаКлиенте
Процедура КомандаСписокОбъектовДляВыгрузкиСнятьПометки(Команда)
...
&НаКлиенте
Процедура КомандаСписокОбъектовДляВыгрузкиОткрытьВРедактореОбъекта(Команда)
	
	#Область Предусловия
	
	ТекущиеДанные = Элементы.СписокОбъектовДляВыгрузки.ТекущиеДанные;
	...
	ПараметрыФормы = Новый Структура("ОбъектБазы", ТекущиеДанные.ОбъектБазыВыгрузка);
...
&НаКлиенте
Процедура КомандаСписокОбъектовДляВыгрузкиУстановитьПометкиВыгружатьПоСсылкам(Команда)
	
	Для каждого стр Из СписокОбъектовДляВыгрузки Цикл
		стр.ВыгружатьПоСсылкам = Истина;
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаСписокОбъектовДляВыгрузкиСнятьПометкиВыгружатьПоСсылкам(Команда)
	
	Для каждого стр Из СписокОбъектовДляВыгрузки Цикл
		стр.ВыгружатьПоСсылкам = Ложь;
	КонецЦикла;
...
Процедура КомандаВыгрузитьДанныеОбъектаXML(Команда)
	...
	ОчиститьСообщения();
	ВсеВыгруженныеОбъекты.Очистить();
...
Процедура ЗагрузитьДанныеОбъектаXMLНаКлиенте()
	...
	ДиалогВыбораФайла.Заголовок = "Выберите файлы для загрузки";
...
&НаКлиенте
Процедура ЗагрузитьДанныеОбъектаXMLНаКлиентеПослеЗакрытияДилога(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура ВыгрузитьДанныеОбъектаXMLНаКлиенте()
	...
	Если Не ЗначениеЗаполнено(ОбъектБазыВыгрузка) И СписокОбъектовДляВыгрузки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	#КонецОбласти
	
	Если Элементы.СтраницыВыгрузка.ТекущаяСтраница = Элементы.СтраницаВыгрузкаОбъекта Тогда
		
		ДиалогВыборСохранение = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
		ДиалогВыборСохранение.МножественныйВыбор = Ложь;
		ДиалогВыборСохранение.Заголовок = "Укажите имя файла для выгрузки данных объекта";
		
		ПолноеИмяФайла = ПолноеИмяФайлаПоДаннымОбъекта(ОбъектБазыВыгрузка);
		ДиалогВыборСохранение.ПолноеИмяФайла = ПолноеИмяФайла;
		
		Если ВыгружатьДвиженияДокумента Или ВыгружатьОбъектыПоСсылкам Или ВыгружатьСсылкиНаОбъект Тогда
			ДиалогВыборСохранение.Фильтр = "Zip-архив (*.zip)|*.zip";
		Иначе
			ДиалогВыборСохранение.Фильтр = "XML файл (*.xml)|*.xml";
		КонецЕсли;
		
		ДиалогВыборСохранение.Показать(Новый ОписаниеОповещения("ВыгрузитьДанныеОбъектаXMLНаКлиентеПослеЗакрытияДилога", ЭтаФорма));
		
	ИначеЕсли Элементы.СтраницыВыгрузка.ТекущаяСтраница = Элементы.СтраницаВыгрузкаСписка Тогда
		
		ДиалогВыборКаталога = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
		ДиалогВыборКаталога.МножественныйВыбор = Ложь;
		ДиалогВыборКаталога.Заголовок = "Укажите каталог для выгрузки списка объектов";
		
		ДиалогВыборКаталога.Показать(Новый ОписаниеОповещения("ВыгрузитьСписокОбъектовXMLНаКлиентеПослеЗакрытияДилога", ЭтаФорма));
...
&НаКлиенте
Процедура ВыгрузитьСписокОбъектовXMLНаКлиентеПослеЗакрытияДилога(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ИмяКаталогаДляВыгрузки = ВыбранныеФайлы[0];
	
	Если Не ЗначениеЗаполнено(ИмяКаталогаДляВыгрузки) Тогда
		Возврат;
	КонецЕсли;
	
	#КонецОбласти
	
	Для каждого стр_СписокОбъектовДляВыгрузки Из СписокОбъектовДляВыгрузки Цикл
		
		#Область Предусловия
		
		Если Не стр_СписокОбъектовДляВыгрузки.Пометка Тогда
			Продолжить;
		КонецЕсли;
		
		#КонецОбласти
		
		ОбъектБазыВыгрузка = стр_СписокОбъектовДляВыгрузки.ОбъектБазыВыгрузка;
		ВыгружатьОбъектыПоСсылкам = стр_СписокОбъектовДляВыгрузки.ВыгружатьПоСсылкам;
		
		ВыгружатьДвиженияДокумента = ВыгружатьДвиженияДокументовПриВыгрузкеСписка;
		
		Если стр_СписокОбъектовДляВыгрузки.ВыгружатьПоСсылкам Тогда
			ИмяФайлаДанных = ИмяКаталогаДляВыгрузки + "\" + ПолноеИмяФайлаПоДаннымОбъекта(ОбъектБазыВыгрузка) + ".zip";
		Иначе
			ИмяФайлаДанных = ИмяКаталогаДляВыгрузки + "\" + ПолноеИмяФайлаПоДаннымОбъекта(ОбъектБазыВыгрузка) + ".xml";
		КонецЕсли;
	...
	ПоказатьОповещениеПользователя(
		"Выгрузка выполнена", 
		Новый ОписаниеОповещения("ВыгрузитьСписокОбъектовXMLНаКлиентеПриНажатии", ЭтаФорма, Новый Структура("ИмяКаталога", ИмяКаталогаДляВыгрузки)), 
		ИмяКаталогаДляВыгрузки, Элементы.КартинкаИнформация.Картинка, СтатусОповещенияПользователя.Информация
	);
...
&НаКлиенте
Процедура ВыгрузитьСписокОбъектовXMLНаКлиентеПриНажатии(ДополнительныеПараметры) Экспорт
	...
	НачатьЗапускПриложения(Новый ОписаниеОповещения("ВыгрузитьСписокОбъектовXMLНаКлиентеПриНажатииПродолжение", ЭтотОбъект), ИмяКаталога);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьСписокОбъектовXMLНаКлиентеПриНажатииПродолжение(КодВозврата, ДополнительныеПараметры) Экспорт
	//
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьДанныеОбъектаXMLПриНажатии(ДополнительныеПараметры) Экспорт
	...
	ИмяФайлаДанных = ДополнительныеПараметры.ИмяФайлаДанных;
	...
	НачатьЗапускПриложения(Новый ОписаниеОповещения("ВыгрузитьДанныеОбъектаXMLПриНажатииПродолжение", ЭтотОбъект), ИмяФайлаДанных);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьДанныеОбъектаXMLПриНажатииПродолжение(КодВозврата, ДополнительныеПараметры) Экспорт
	//
КонецПроцедуры
...
Процедура ЗагрузитьДанныеОбъектаXML(АдресФайла, РасширениеФайла)
	...
	Если ВРег(РасширениеФайла) = ВРег(".xml") Тогда
		...
		ВключитьЗагруженныйОбъектВСписокЗагруженных(ДанныеОбъектаБазы.Ссылка);
...
&НаСервере
Процедура ВключитьЗагруженныйОбъектВСписокЗагруженных(ЗагруженныйОбъект)
	
	#Область Предусловия
	
	Если Не ЗначениеЗаполнено(ЗагруженныйОбъект) Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектыБазыЗагрузкаСтроки = ОбъектыБазыЗагрузка.НайтиСтроки(Новый Структура("ОбъектБазыЗагрузка", ЗагруженныйОбъект));
	
	Если ОбъектыБазыЗагрузкаСтроки.Количество() > 0 Тогда
		Возврат;
	КонецЕсли;
	
	#КонецОбласти
	
	стр_ОбъектыБазыЗагрузка = ОбъектыБазыЗагрузка.Добавить();
	стр_ОбъектыБазыЗагрузка.ОбъектБазыЗагрузка = ЗагруженныйОбъект;
...
Процедура ВыгрузитьДанныеОбъектаXMLВФайл(СсылкаНаЭлемент, ИмяВременногоФайла, ВыгрузкаВыполнена)
	...
	Если Не (ВсеВыгруженныеОбъекты.НайтиПоЗначению(СсылкаНаЭлемент) = Неопределено) Тогда
		Возврат;
	КонецЕсли;
	...
	ВсеВыгруженныеОбъекты.Добавить(СсылкаНаЭлемент);
...
Процедура ВыгрузитьДанныеОбъектаXML(АдресВременногоХранилища)
	...
	// Выгрузить дополнительные данные объекта.
	...
	// Записать файлы, удалить временные файлы.
	...
	// Поместить файл с данными вовременное хранилище.
...
Процедура УстановитьСвойстваЭлементовФормы(Форма)
	...
	Форма.ОбъектыБазыЗагрузкаКоличество = Форма.ОбъектыБазыЗагрузка.Количество();
...
Функция ЭтоСтрокаКлючаЗаписиРегистра(ИмяТипа)
	...
	Если Не ЗначениеЗаполнено(ИмяТипа) Тогда
		ВызватьИсключение("ЭтоСтрокаКлючаЗаписиРегистра(): Не указано значение параметра <ИмяТипа>.");
	КонецЕсли;
	...
	ЧастиСтроки = СтрРазделить(ИмяТипа, ".", Ложь);
...
Функция ПолноеИмяФайлаПоДаннымОбъекта(СсылкаНаОбъект)
	...
	Если ИмяТипа = "Документ" Тогда
		...
		ПолноеИмяФайла = СтрШаблон("%1.%2.%3.%4.%5", 
			ИмяТипа, ИмяОбъекта, ЗначенияРеквизитов.Номер, Формат(ЗначенияРеквизитов.Дата, "ДФ=ddMMyyyy-HHmmss"), ВерсияДанных);
	Иначе
		...
		Исключение
			ЗначенияРеквизитов.Код = "nc";
		КонецПопытки;
		
		ПолноеИмяФайла = СтрШаблон("%1.%2.%3.%4", ИмяТипа, ИмяОбъекта, "Код", ВерсияДанных);
		
	КонецЕсли;
	
	Результат = ПолноеИмяФайла; // "Документ.ПриходнаяНакладная.000000002.21052020-180442.AAAACgAAAAA="
	Возврат Результат;
...
Функция ПредставлениеНайденнойСсылки(НайденнаяСсылка)
	...
	ПредставлениеСсылки = СтрШаблон("%1 (%2)", НайденнаяСсылка, ИмяТипа);
