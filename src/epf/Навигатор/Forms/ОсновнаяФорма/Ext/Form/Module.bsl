...
Процедура КомандаОткрытьМакет(Команда)
	...
	//#КонецОбласти
	//
	//ТабличныйДокумент = ПолучитьТабличныйДокументМакета();
	//ТабличныйДокумент.Показать();
	//
КонецПроцедуры

&НаСервере
Функция ПолучитьТабличныйДокументМакета()
	
	Результат = Новый ТабличныйДокумент;
	Результат.Вывести(ПолучитьМакетОбъектаМетаданных());
...
&НаСервере
Функция ПолучитьМакетОбъектаМетаданных()
	
	Результат = Документы.АвансовыйОтчет.ПолучитьМакет("ПФ_MXL_АвансовыйОтчет_ru");
	Возврат Результат;
	
КонецФункции

&НаКлиенте
Процедура КомандаОткрытьФормуСПараметромАвтоТест(Команда)
	...
	Если Не (ТекущиеДанные.ИмяЭлементаКоллекции = "Форма") Тогда
		Возврат;
	КонецЕсли;
	
	#КонецОбласти
	
	ПараметрыОткрытияФормы = Новый Структура("АвтоТест", Истина);
	
	ОткрытьФорму(ТекущиеДанные.ИмяФормы, ПараметрыОткрытияФормы);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаОткрытьОтчетОбработкуФорму(Команда)
	...
	Если Не (ТекущиеДанные.ИмяЭлементаКоллекции = "Отчет" 
		Или ТекущиеДанные.ИмяЭлементаКоллекции = "Обработка"
		Или ТекущиеДанные.ИмяЭлементаКоллекции = "Форма") Тогда
	...
	ИначеЕсли ТекущиеДанные.ИмяЭлементаКоллекции = "Форма" Тогда
		
		ОткрытьФорму(ТекущиеДанные.ИмяФормы);
...
Процедура СтруктураМетаданныхКонфигурацииПриАктивизацииСтроки(Элемент)
	...
	Если Не ЗначениеЗаполнено(КлючТекущейСтрокиТаблицыМетаданных) Тогда
		КлючТекущейСтрокиТаблицыМетаданных = ТекущиеДанные.ПолноеИмя;
	Иначе
		Если КлючТекущейСтрокиТаблицыМетаданных = ТекущиеДанные.ПолноеИмя Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтобразитьСтруктуруОбъектаМетаданных(ИдСтроки)
	...
	Для каждого КлючЗначение Из СтруктураОбъектаМД Цикл
		...
		СтрокаКоллекции.Имя                  = КлючЗначение.Ключ; // "Макеты"
		СтрокаКоллекции.Представление        = КлючЗначение.Ключ; // "Макеты"
...
Процедура УстановитьВидимостьКомандКонтекстногоМенюТаблицы(ТаблицаФормы)
	...
	Если ТаблицаФормы.Имя = "СтруктураМетаданныхКонфигурации" Тогда
		...
		ПредохранительГлубины = 3;
		УстановитьВидимостьКомандКонтекстногоМенюТаблицыРекурсивно(ТаблицаФормы, ТаблицаФормы.КонтекстноеМеню.ПодчиненныеЭлементы, ПредохранительГлубины);
...
&НаКлиенте
Процедура УстановитьВидимостьКомандКонтекстногоМенюТаблицыРекурсивно(ТаблицаФормы, КоллекцияЭлементов, ПредохранительГлубины)
	
	Если ПредохранительГлубины = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПредохранительГлубины = ПредохранительГлубины - 1;
	
	Для каждого ПодчиненныйЭлемент Из КоллекцияЭлементов Цикл
		
		Если ТипЗнч(ПодчиненныйЭлемент) = Тип("КнопкаФормы") Тогда
			
			КнопкаФормы = ПодчиненныйЭлемент;
			КнопкаФормы.Видимость = Ложь;
			
			ИмяЭлементаКоллекции = ТаблицаФормы.ТекущиеДанные.ИмяЭлементаКоллекции;
			
			Если ИмяЭлементаКоллекции = "Отчет" Или ИмяЭлементаКоллекции = "Обработка" Тогда
				
				Если КнопкаФормы.Имя = "КомандаОткрытьОтчетОбработкуФорму" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
				
				Если КнопкаФормы.Имя = "КомандаОтобразитьСвойстваОбъектаМетаданных" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
				
				Если КнопкаФормы.Имя = "КомандаОтобразитьСтруктуруОбъектаМетаданных" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
			...
			ИначеЕсли ИмяЭлементаКоллекции = "Форма" Тогда
				
				Если КнопкаФормы.Имя = "КомандаОткрытьОтчетОбработкуФорму" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
				
				Если КнопкаФормы.Имя = "КомандаОткрытьФормуСПараметромАвтоТест" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
			...
			ИначеЕсли ИмяЭлементаКоллекции = "Документ" Или ИмяЭлементаКоллекции = "Справочник" 
				Или ИмяЭлементаКоллекции = "РегистрСведений" Или ИмяЭлементаКоллекции = "РегистрНакопления" Или ИмяЭлементаКоллекции = "РегистрБухгалтерии" 
				Или ИмяЭлементаКоллекции = "РегистрРасчета"
				Тогда
				
				Если КнопкаФормы.Имя = "КомандаОткрытьСписокЭлементовОбъектаМетаданных" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
				
				Если КнопкаФормы.Имя = "КомандаОтобразитьСтруктуруХраненияОбъектаМетаданных" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
				
				Если КнопкаФормы.Имя = "КомандаОтобразитьКоличество" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
			...
			ИначеЕсли ИмяЭлементаКоллекции = "Макет" Тогда
				
				Если КнопкаФормы.Имя = "КомандаОткрытьМакет" Тогда
					КнопкаФормы.Видимость = Истина;
				КонецЕсли;
		...
		ИначеЕсли ТипЗнч(ПодчиненныйЭлемент) = Тип("ГруппаФормы") Тогда
			...
			ГруппаФормы = ПодчиненныйЭлемент;
			
			УстановитьВидимостьКомандКонтекстногоМенюТаблицыРекурсивно(ТаблицаФормы, ГруппаФормы.ПодчиненныеЭлементы, ПредохранительГлубины);
