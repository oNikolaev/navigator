...
Функция СтруктураОбъектаМетаданных(ИмяКоллекции, ИмяОбъектаМетаданных) Экспорт
	...
	ИмяТипаОбъекта = ИмяТипаОбъектаМетаданных(ОбъектМетаданных); // "Справочник"
	
	КоллекцииСвойств = КоллекцииСвойствОбъектаМетаданных(); // Массив
	
	Для каждого КоллекцияСвойств Из КоллекцииСвойств Цикл
		
		Попытка
			КоллекцияСвойствМд = ОбъектМетаданных[КоллекцияСвойств.Имя]; // ОписанияСтандартныхРеквизитов
		Исключение
			Продолжить;
		КонецПопытки;
		...
		Если ТипЗнч(КоллекцияСвойствМд) = Тип("ОписанияСтандартныхРеквизитов")
			...
			Для каждого ЭлементКоллекции Из КоллекцияСвойствМд Цикл
				...
				ДанныеЭлементаКоллекции = Новый Структура("ТипЗаписи,Имя,Синоним,Комментарий,ИмяФормы,ДанныеКоллекции,ПолноеИмя");
...
Функция НавигационнаяСсылкаНаЭлемент(ЭлементБазы) Экспорт
	...
	Попытка
		НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(ЭлементБазы);
	Исключение
		//
	КонецПопытки;
	...
	Если Не ЗначениеЗаполнено(НавигационнаяСсылка) Тогда
		...
		Если ЭтоПланОбмена(ЭлементБазыМД) Тогда
			НавигационнаяСсылка = НавигационнаяСсылкаНаУзелПланаОбмена(ЭлементБазы);
		КонецЕсли;
...
Функция ПараметрыТаблицыЗаписиПоИмениЭлемента(ИмяЭлемента) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяЭлемента) Тогда
		ВызватьИсключение("ПараметрыТаблицыЗаписиПоИмениЭлемента(): Не указан параметр <ИмяЭлемента>.");
	КонецЕсли;
	...
	Если ЧастиИмени.Количество() = 0 Тогда
		ВызватьИсключение("ПараметрыТаблицыЗаписиПоИмениЭлемента(): Неверный формат параметра <ИмяЭлемента>.");
	КонецЕсли;
	
	Если ЧастиИмени.Количество() < 7 Тогда
		ВызватьИсключение("ПараметрыТаблицыЗаписиПоИмениЭлемента(): Некорректные данные параметра <ИмяЭлемента>.");
	КонецЕсли;
...
Функция МетаданныеРегистраПоОписанию(ПараметрыТаблицы) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыТаблицы.ТипТаблицы) Тогда
		ВызватьИсключение("Не заполнено поле <ТипТаблицы> в структуре <ПараметрыТаблицы>.");
	КонецЕсли;
